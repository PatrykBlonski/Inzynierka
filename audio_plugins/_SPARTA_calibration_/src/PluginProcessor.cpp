...
void PluginProcessor::setParameter (int index, float newValue)
{
    ...
    if(index < k_NumOfParameters){
        switch (index) {
            case k_numInputs:  calibration_setNumSources(hPan, (int)(newValue*(float)(MAX_NUM_INPUTS)+0.5)); break;
            case k_numOutputs: calibration_setNumLoudspeakers(hPan, (int)(newValue*(float)(MAX_NUM_OUTPUTS)+0.5)); break;
        }
    }
    /* source direction parameters */
        ...
        if (!(index % 2)){
            ...
            if (newValueScaled != calibration_getSourceAzi_deg(hPan, index/2)){
                calibration_setSourceAzi_deg(hPan, index/2, newValueScaled);
                refreshWindow = true;
            }
        }
        else{
            newValueScaled = (newValue - 0.5f)*180.0f;
            if (newValueScaled != calibration_getSourceElev_deg(hPan, index/2)){
                calibration_setSourceElev_deg(hPan, index/2, newValueScaled);
                refreshWindow = true;
            }
        }
    }
    /* loudspeaker direction parameters */
        ...
        if (index % 3 == 0) {
            // Azimuth
            newValueScaled = (newValue - 0.5f) * 360.0f;
            if (newValueScaled != calibration_getLoudspeakerAzi_deg(hPan, loudspeakerIndex)) {
                calibration_setLoudspeakerAzi_deg(hPan, loudspeakerIndex, newValueScaled);
                refreshWindow = true;
            }
        }
        else if (index % 3 == 1) {
            // Elevation
            newValueScaled = (newValue - 0.5f) * 180.0f;
            if (newValueScaled != calibration_getLoudspeakerElev_deg(hPan, loudspeakerIndex)) {
                calibration_setLoudspeakerElev_deg(hPan, loudspeakerIndex, newValueScaled);
                refreshWindow = true;
            }
        }
        else {
            ...
            if (newValueScaled != calibration_getLoudspeakerDist_deg(hPan, loudspeakerIndex)) {
                calibration_setLoudspeakerDist_deg(hPan, loudspeakerIndex, newValueScaled);
                refreshWindow = true;
            }
        }
    }
...
float PluginProcessor::getParameter (int index)
{
    ...
    if(index < k_NumOfParameters){
        switch (index) {
            case k_numInputs:  return (float)(calibration_getNumSources(hPan))/(float)(MAX_NUM_INPUTS);
            case k_numOutputs: return (float)(calibration_getNumLoudspeakers(hPan))/(float)(MAX_NUM_OUTPUTS);
            default: return 0.0f;
        }
    }
    /* source direction parameters */
        ...
        index-=k_NumOfParameters;
        if (!(index % 2))
            return (calibration_getSourceAzi_deg(hPan, index/2)/360.0f) + 0.5f;
        else
            return (calibration_getSourceElev_deg(hPan, (index-1)/2)/180.0f) + 0.5f;
    }
    /* loudspeaker direction parameters */
    else{
        index -= (k_NumOfParameters+2*MAX_NUM_INPUTS);
        if (!(index % 2))
            return (calibration_getLoudspeakerAzi_deg(hPan, index/2)/360.0f) + 0.5f;
        else
            return (calibration_getLoudspeakerElev_deg(hPan, (index-1)/2)/180.0f) + 0.5f;
    }
}
...
const String PluginProcessor::getParameterText(int index)
{
    ...
    if(index < k_NumOfParameters){
        switch (index) {
            case k_numInputs:  return String(calibration_getNumSources(hPan));
            case k_numOutputs: return String(calibration_getNumLoudspeakers(hPan));
            default: return "NULL";
        }
    }
    /* source direction parameters */
        ...
        index -= k_NumOfParameters;
        if (!(index % 2))
            return String(calibration_getSourceAzi_deg(hPan, index/2));
        else
            return String(calibration_getSourceElev_deg(hPan, (index-1)/2));
    }
    /* loudspeaker direction parameters */
    else{
        index -= (k_NumOfParameters+2*MAX_NUM_INPUTS);
        if (!(index % 2))
            return String(calibration_getLoudspeakerAzi_deg(hPan, index/2));
        else
            return String(calibration_getLoudspeakerElev_deg(hPan, (index-1)/2));
    }
}
...
void PluginProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
	...
	calibration_init(hPan, nSampleRate);
    powermap_init(hPm, nSampleRate);
    AudioProcessor::setLatencySamples(calibration_getProcessingDelay());
}
...
void PluginProcessor::generateSine(const double deltaT, AudioBuffer<float>& buffer) {
    int numberOfInputs = calibration_getNumSources(hPan);
...
void PluginProcessor::distanceCalculation(AudioBuffer<float>& sweep, AudioBuffer<float>& input, int loudNum)
{
    ...
    //calibration_setLoudspeakerDist_deg(hPan, loudNum, distance);
    ...
    for (auto& elem : norm) {
        calibration_setLoudspeakerDist_plot(hPan, i, 0.7 + 0.3 * (elem - minNorm) / (maxNorm - minNorm));
        calibration_setLoudspeakerDist_deg(hPan, i, dists[i] - avgDist);
        i++;
    }
...
{
    XmlElement xml("calibrationPLUGINSETTINGS");
    for(int i=0; i<calibration_getMaxNumSources(); i++){
        xml.setAttribute("SourceAziDeg" + String(i), calibration_getSourceAzi_deg(hPan,i));
        xml.setAttribute("SourceElevDeg" + String(i), calibration_getSourceElev_deg(hPan,i));
    }
    xml.setAttribute("nSources", calibration_getNumSources(hPan));
    
    xml.setAttribute("JSONFilePath", lastDir.getFullPathName());
    
    for(int i=0; i<calibration_getMaxNumLoudspeakers(); i++){
        xml.setAttribute("LoudspeakerAziDeg" + String(i), calibration_getLoudspeakerAzi_deg(hPan,i));
        xml.setAttribute("LoudspeakerElevDeg" + String(i), calibration_getLoudspeakerElev_deg(hPan,i));
    }
    xml.setAttribute("nLoudspeakers", calibration_getNumLoudspeakers(hPan));
...
void PluginProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    if (xmlState != nullptr) {
        if (xmlState->hasTagName("calibrationPLUGINSETTINGS")) {
            for(int i=0; i<calibration_getMaxNumSources(); i++){
                if(xmlState->hasAttribute("SourceAziDeg" + String(i)))
                    calibration_setSourceAzi_deg(hPan, i, (float)xmlState->getDoubleAttribute("SourceAziDeg" + String(i), 0.0f));
                if(xmlState->hasAttribute("SourceElevDeg" + String(i)))
                    calibration_setSourceElev_deg(hPan, i, (float)xmlState->getDoubleAttribute("SourceElevDeg" + String(i), 0.0f));
            }
            if(xmlState->hasAttribute("nSources"))
                calibration_setNumSources(hPan, xmlState->getIntAttribute("nSources", 1));
            ...
            for(int i=0; i<calibration_getMaxNumLoudspeakers(); i++){
                if(xmlState->hasAttribute("LoudspeakerAziDeg" + String(i)))
                    calibration_setLoudspeakerAzi_deg(hPan, i, (float)xmlState->getDoubleAttribute("LoudspeakerAziDeg" + String(i),0.0f));
                if(xmlState->hasAttribute("LoudspeakerElevDeg" + String(i)))
                    calibration_setLoudspeakerElev_deg(hPan, i, (float)xmlState->getDoubleAttribute("LoudspeakerElevDeg" + String(i), 0.0f));
            }
            if(xmlState->hasAttribute("nLoudspeakers"))
                calibration_setNumLoudspeakers(hPan, xmlState->getIntAttribute("nLoudspeakers", 1));
            
            calibration_refreshSettings(hPan);
        }
    }
}
...
void PluginProcessor::saveConfigurationToFile (File destination, int srcOrLs)
{
    ...
    switch (srcOrLs){
        case 0:{
            for (int i=0; i<calibration_getNumSources(hPan);i++) {
                elements.appendChild (ConfigurationHelper::
                                      createElement(calibration_getSourceAzi_deg(hPan, i),
                                                    calibration_getSourceElev_deg(hPan, i),
                                                    1.0f, i+1, false, 1.0f), nullptr);
            }
        }
        break;
        case 1:{
            for (int i=0; i<calibration_getNumLoudspeakers(hPan);i++) {
                elements.appendChild (ConfigurationHelper::
                                     createElement(calibration_getLoudspeakerAzi_deg(hPan, i),
                                                       calibration_getLoudspeakerElev_deg(hPan, i),
                                                       calibration_getLoudspeakerDist_deg(hPan, i), i + 1, false, 1.0f), nullptr);
            }
        }
        break;
    }
...
void PluginProcessor::loadConfiguration (const File& configFile, int srcOrLs)
{
    ...
    if(result.wasOk()){
        ...
        switch(srcOrLs){
            case 0:{
                calibration_setNumSources(hPan, num_el);
                for (ValueTree::Iterator it = elements.begin() ; it != elements.end(); ++it){
                    if ( !((*it).getProperty("Imaginary"))){
                        calibration_setSourceAzi_deg(hPan, channelIDs[el_idx]-1, (*it).getProperty("Azimuth"));
                        calibration_setSourceElev_deg(hPan, channelIDs[el_idx]-1, (*it).getProperty("Elevation"));
                        el_idx++;
                    }
                }
            }
            break;
            case 1:{
                calibration_setNumLoudspeakers(hPan, num_el);
                for (ValueTree::Iterator it = elements.begin() ; it != elements.end(); ++it){
                    if ( !((*it).getProperty("Imaginary"))){
                        calibration_setLoudspeakerAzi_deg(hPan, channelIDs[el_idx]-1, (*it).getProperty("Azimuth"));
                        calibration_setLoudspeakerDist_deg(hPan, channelIDs[el_idx] - 1, (*it).getProperty("Distance")); //XXXX
                        calibration_setLoudspeakerElev_deg(hPan, channelIDs[el_idx]-1, (*it).getProperty("Elevation"));
                        el_idx++;
                    }
                }
            }
