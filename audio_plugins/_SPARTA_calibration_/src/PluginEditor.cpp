    ...
    : AudioProcessorEditor(ownerFilter), progressbar(progress)
{
   ...
   // return pData->loudpkrs_dirs_deg[index][0];
    ...
    loudspeakerCoordsView_handle = new outputCoordsView(ownerFilter, MAX_NUM_OUTPUTS, calibration_getNumLoudspeakers(hPan));
    ...
    loudspeakerCoordsView_handle->setNCH(calibration_getNumLoudspeakers(hPan));

    /* grab current parameter settings */
    SL_num_sources->setValue(calibration_getNumSources(hPan),dontSendNotification);
    TB_showOutputs->setToggleState(true, dontSendNotification);
    TB_PlayAll->setToggleState(true, dontSendNotification);
    hVst->playAll = true;

    CBformat->setSelectedId(calibration_getChOrder(hPan), dontSendNotification);
    CBnorm->setSelectedId(calibration_getNormType(hPan), dontSendNotification);
    CBformat->setItemEnabled(CH_FUMA, true);
    CBnorm->setItemEnabled(NORM_FUMA, true);


    /* create panning window */
    panWindow.reset (new calibrationView(ownerFilter, 460, 460));
...
void PluginEditor::paint (juce::Graphics& g)
{
    ...
    switch (currentWarning){
        ...
        case k_warning_frameSize:
            g.drawText(TRANS("Set frame size to multiple of ") + String(calibration_getFrameSize()),
                       getBounds().getWidth()-225, 16, 530, 11,
                       Justification::centredLeft, true);
            break;
        case k_warning_supported_fs:
            g.drawText(TRANS("Sample rate (") + String(calibration_getDAWsamplerate(hPan)) + TRANS(") is unsupported"),
                       getBounds().getWidth()-225, 16, 530, 11,
                       Justification::centredLeft, true);
            break;
        case k_warning_NinputCH:
            g.drawText(TRANS("Insufficient number of input channels (") + String(hVst->getTotalNumInputChannels()) +
                       TRANS("/") + String(calibration_getNumSources(hPan)) + TRANS(")"),
                       getBounds().getWidth()-225, 16, 530, 11,
                       Justification::centredLeft, true);
            break;
        case k_warning_NoutputCH:
            g.drawText(TRANS("Insufficient number of output channels (") + String(hVst->getTotalNumOutputChannels()) +
                       TRANS("/") + String(calibration_getNumLoudspeakers(hPan)) + TRANS(")"),
                       getBounds().getWidth()-225, 16, 530, 11,
                       Justification::centredLeft, true);
            break;
    }
...
void PluginEditor::comboBoxChanged (juce::ComboBox* comboBoxThatHasChanged)
{
    ...
    //    calibration_setInputConfigPreset(hPan, CBsourceDirsPreset->getSelectedId());
    //    refreshPanViewWindow = true;
    ...
    if (comboBoxThatHasChanged == CBformat.get())
    {
        ...
        calibration_setChOrder(hPan, CBformat->getSelectedId());
    ...
    else if (comboBoxThatHasChanged == CBnorm.get())
    {
        ...
        calibration_setNormType(hPan, CBnorm->getSelectedId());
...
void PluginEditor::sliderValueChanged (juce::Slider* sliderThatWasMoved)
{
    ...
    //    calibration_setNumSources(hPan, (int)SL_num_sources->getValue());
    //    refreshPanViewWindow = true;
    ...
    if (sliderThatWasMoved == SL_num_loudspeakers.get())
    {
        ...
        calibration_setNumLoudspeakers(hPan, (int)SL_num_loudspeakers->getValue());
        for (int i=0; i < (int)SL_num_loudspeakers->getValue();i++)
        {
            calibration_setLoudspeakerAzi_deg(hPan, i, 0.0f);
            calibration_setLoudspeakerElev_deg(hPan, i, 0.0f);
            calibration_setLoudspeakerDist_deg(hPan, i, 0.0f);
            calibration_setLoudspeakerDist_plot(hPan, i, 0.0f);
        }
...
void PluginEditor::buttonClicked (juce::Button* buttonThatWasClicked)
{
    ...
    else if (buttonThatWasClicked == tb_play.get())
    {
        ...
        if(hVst->loudspeakerNumber < calibration_getNumLoudspeakers(hPan) && hVst->loudspeakerNumber != 0 && !TB_PlayAll->getToggleState())
            hVst->next();
    }
...
void PluginEditor::timerCallback(int timerID)
{
    switch(timerID){
        ...
        case TIMER_GUI_RELATED:
            ...
            loudspeakerCoordsView_handle->setNCH(calibration_getNumLoudspeakers(hPan));
            SL_num_sources->setValue(calibration_getNumSources(hPan),dontSendNotification);
            SL_num_loudspeakers->setValue(calibration_getNumLoudspeakers(hPan),dontSendNotification);


            /* Progress bar */
#if 0
            if(calibration_getCodecStatus(hPan)==CODEC_STATUS_INITIALISING){
                addAndMakeVisible(progressbar);
                progress = (double)calibration_getProgressBar0_1(hPan);
                char text[calibration_PROGRESSBARTEXT_CHAR_LENGTH];
                calibration_getProgressBarText(hPan, (char*)text);
          ...
          /*  if ((hVst->getCurrentBlockSize() % calibration_getFrameSize()) != 0){
                currentWarning = k_warning_frameSize;
                repaint(0,0,getWidth(),32);
            }*/
            else if ( !((calibration_getDAWsamplerate(hPan) == 48e3)) ){
                currentWarning = k_warning_supported_fs;
                repaint(0,0,getWidth(),32);
            }
            else if ((hVst->getCurrentNumInputs() < calibration_getNumSources(hPan))){
                currentWarning = k_warning_NinputCH;
                repaint(0,0,getWidth(),32);
            }
            else if ((hVst->getCurrentNumOutputs() < calibration_getNumLoudspeakers(hPan))){
